--  Managing List of QoS Entries withautomatic IP assignment.
--
...
--TODO: schoener mit index calculation machen
local function getIndex(IP, prio)
...
local function deleteDNSassignment(hostname)
  ...
  os.execute("sed -i '/"..hostname.."/d' /etc/hosts") -- > /etc/hosts")
...
function QoSMLib.printPrioList(prio)
  ...
  local maxLength = 0 -- get the length of longest hostname or "hostname" for propper aligning
  for key,value in pairs(db) do
